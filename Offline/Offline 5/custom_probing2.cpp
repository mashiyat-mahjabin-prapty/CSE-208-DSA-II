...
int main()
{
	...
	cin >> n;
	int collisions=0;
	custom_probing2 c2(n);
	for(int i = 0; i < 10000; i++)
	{
		collisions += c2.insert(words[i], i+1);
	}
	
	cout << "No. of collisions: " << collisions << endl;
	cout << "No. of unsuccessful insert: " << c2.get_u() << endl;
	
	int found=0;
	for(int i = 0; i < 1000; i++)
	{
		found+=c2.search(searching[i]);
	}
	cout << endl;
